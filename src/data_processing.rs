...
/// Generate world data and export as Sosaia binary chunks instead of Minecraft format.
pub fn generate_world_for_export(
    elements: Vec<ProcessedElement>,
    xzbbox: XZBBox,
    llbbox: LLBBox,
    ground: Ground,
    ...
    city_name: &str,
) -> Result<crate::chunk_export::CityManifest, String> {
    let mut editor: WorldEditor = WorldEditor::new_with_format_and_name(
        options.path,
        &xzbbox,
        llbbox,
    ...
    let process_pb = ProgressBar::new(elements_count as u64);
    process_pb.set_style(ProgressStyle::default_bar()
    ...
    let suppressed_building_outlines: HashSet<u64> = {
        ...
        for element in &elements {
            if let ProcessedElement::Relation(rel) = element {
                let is_building_type = rel.tags.get("type").map(|t| t.as_str()) == Some("building");
                if is_building_type {
                    let has_parts = rel.members.iter().any(|m| m.role == ProcessedMemberRole::Part);
                    if has_parts {
                        for member in &rel.members {
                            if member.role == ProcessedMemberRole::Outer {
                                outlines.insert(member.way.id);
    ...
    // Process all elements (same logic as generate_world_with_options)
    for element in elements.into_iter() {
        process_pb.inc(1);
        match &element {
            ProcessedElement::Way(way) => {
                if way.tags.contains_key("building") || way.tags.contains_key("building:part") {
                    if !suppressed_building_outlines.contains(&way.id) {
                        buildings::generate_buildings(&mut editor, way, args, None, None, &flood_fill_cache);
                    }
                } else if way.tags.contains_key("highway") {
                    highways::generate_highways(&mut editor, &element, args, &highway_connectivity, &flood_fill_cache);
                } else if way.tags.contains_key("landuse") {
                    landuse::generate_landuse(&mut editor, way, args, &flood_fill_cache, &building_footprints);
                } else if way.tags.contains_key("natural") {
                    natural::generate_natural(&mut editor, &element, args, &flood_fill_cache, &building_footprints);
                } else if way.tags.contains_key("amenity") {
                    amenities::generate_amenities(&mut editor, &element, args, &flood_fill_cache);
                } else if way.tags.contains_key("leisure") {
                    leisure::generate_leisure(&mut editor, way, args, &flood_fill_cache, &building_footprints);
                } else if way.tags.contains_key("barrier") {
                    barriers::generate_barriers(&mut editor, &element);
                } else if let Some(val) = way.tags.get("waterway") {
                    if val == "dock" {
            ...
            ProcessedElement::Node(node) => {
                if node.tags.contains_key("door") || node.tags.contains_key("entrance") {
                    doors::generate_doors(&mut editor, node);
                } else if node.tags.get("natural") == Some(&"tree".to_string()) {
                    natural::generate_natural(&mut editor, &element, args, &flood_fill_cache, &building_footprints);
                } else if node.tags.contains_key("amenity") {
                    amenities::generate_amenities(&mut editor, &element, args, &flood_fill_cache);
                } else if node.tags.contains_key("barrier") {
            ...
            ProcessedElement::Relation(rel) => {
                let is_building_relation = rel.tags.contains_key("building")
                    ...
                    || rel.tags.get("type").map(|t| t.as_str()) == Some("building");
                if is_building_relation {
                    buildings::generate_building_from_relation(&mut editor, rel, args, &flood_fill_cache, &xzbbox);
                } else if rel.tags.contains_key("water")
                    || rel.tags.get("natural").map(|val| val == "water" || val == "bay").unwrap_or(false)
                {
                    water_areas::generate_water_areas_from_relation(&mut editor, rel, &xzbbox);
                } else if rel.tags.contains_key("natural") {
                    natural::generate_natural_from_relation(&mut editor, rel, args, &flood_fill_cache, &building_footprints);
                } else if rel.tags.contains_key("landuse") {
                    landuse::generate_landuse_from_relation(&mut editor, rel, args, &flood_fill_cache, &building_footprints);
                } else if rel.tags.get("leisure") == Some(&"park".to_string()) {
                    leisure::generate_leisure_from_relation(&mut editor, rel, args, &flood_fill_cache, &building_footprints);
                } else if rel.tags.contains_key("man_made") {
                    man_made::generate_man_made(&mut editor, &element, args);
                }
                let way_ids: Vec<u64> = rel.members.iter().map(|m| m.way.id).collect();
        ...
        for chunk_z in min_chunk_z..=max_chunk_z {
            ...
            for x in chunk_min_x..=chunk_max_x {
                for z in chunk_min_z..=chunk_max_z {
                    ...
                    if args.fillground {
                        editor.fill_column_absolute(STONE, x, z, MIN_Y + 1, ground_y - 3, true);
                    }
                    editor.set_block_absolute(BEDROCK, x, MIN_Y, z, None, Some(&[BEDROCK]));
                }
            }
    ...
    // Export as Sosaia chunks instead of saving as Minecraft
    println!("{} Exporting chunks...", "[7/7]".bold());
    ...
    // Access the internal world data for chunk export
    let world_data = editor.get_world_data();
    crate::chunk_export::export_chunks(
        world_data,
        xzbbox.min_x(),
        xzbbox.max_x(),
        xzbbox.min_z(),
